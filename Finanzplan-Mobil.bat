...
REM --- PRIME 2.0 DESKTOP ICONS (NON-NESTED EDITION) ---
echo.
...
REM Wir nutzen KEINEN IF-BLOCK mehr, um Klammer-Fehler zu vermeiden
powershell -NoProfile -Command "$d=[Environment]::GetFolderPath('Desktop');$ws=New-Object -ComObject WScript.Shell;$curr=(Get-Location).Path;$root=(Get-Item $curr).Parent.FullName;$s1=$ws.CreateShortcut((Join-Path $d 'Finanzplan Dashboard.lnk'));$s1.TargetPath=(Join-Path $curr 'native-dist\start-dashboard.bat');$s1.WorkingDirectory=(Join-Path $curr 'native-dist');$s1.IconLocation='shell32.dll, 167';$s1.Save();$s2=$ws.CreateShortcut((Join-Path $d 'Finanzplan Wartung.lnk'));$s2.TargetPath=(Join-Path $root 'Finanzplan-Mobil.bat');$s2.WorkingDirectory=$root;$s2.IconLocation='shell32.dll, 71';$s2.Save();if($?){Write-Host '[+] Duo-Icons erfolgreich erstellt!' -ForegroundColor Green}else{Write-Host '[FEHLER] Icons fehlgeschlagen' -ForegroundColor Red;pause}"
